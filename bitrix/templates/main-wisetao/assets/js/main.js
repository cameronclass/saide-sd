...
document.addEventListener("DOMContentLoaded", () => {
    ...
    dataButtons.forEach((button) => {
        ...
        // Функция для обработки параметра more
        function processMoreParam(url) {
        try {
        var urlObj = new URL(url, window.location.origin);
    } catch (e) {
        // Если URL относительный, создаём временный элемент <a>
        var a = document.createElement('a');
        a.href = url;
        var urlObj = new URL(a.href);
    }

        // Если в строке запроса есть параметр more
        var moreVal = urlObj.searchParams.get('more');
        if (moreVal !== null) {
        // Если до подстроки "?more=" на оригинальном URL символ '/'
        // (например, если URL содержит "/?more="), то считаем, что этот запрос обрабатывается отдельно
        var origUrl = url;
        var idx = origUrl.indexOf('?more=');
        // Проверяем: если idx не равен 0 и символ перед "?more=" равен '/' – пропускаем обработку.
        if (idx > 0 && origUrl.charAt(idx - 1) === '/') {
        return url; // ничего не делаем
    }

        // Удаляем параметр more из URL
        urlObj.searchParams.delete('more');

        // Формируем новый путь:
        // Берем базовый путь страницы (urlObj.pathname) и, если он заканчивается на '/', убираем её.
        var basePath = urlObj.pathname;
        if (basePath.substr(-1) === '/') {
        basePath = basePath.slice(0, -1);
    }

        // Если значение moreVal начинается с '/', то добавляем его как есть, иначе – добавляем '/' перед значением.
        if (moreVal.charAt(0) !== '/') {
        moreVal = '/' + moreVal;
    }

        // Новый путь получается как basePath + moreVal
        urlObj.pathname = basePath + moreVal;
    }
        ...
        // Функция для удаления параметра clear_cache (если он есть)
        function removeClearCache(url) {
        try {
        var urlObj = new URL(url, window.location.origin);
    } catch (e) {
        var a = document.createElement('a');
        ...
        urlObj.searchParams.delete('clear_cache');
        return urlObj.toString();
    }

        // Главный обработчик клика по ссылкам
        document.addEventListener('click', function(e) {
        // Ищем ближайший элемент <a>, ведь клик мог произойти по вложенному элементу
        ...
        // Игнорируем ссылки-анкеры и javascript:
        if (href.indexOf('#') === 0 || href.indexOf('javascript:') === 0) return;

        // Если ссылка должна обрабатываться через AJAX (содержит "/?more=" в начале),
        // оставляем её без изменений
        if (href.indexOf('/?more=') === 0) {
        return; // стандартный переход
    }

        // Флаг, определяющий, нужно ли менять URL
        var processUrl = false;

        // Проверяем наличие параметра more:
        if (href.indexOf('?more=') !== -1 || href.indexOf('&more=') !== -1) {
        processUrl = true;
    }
        // Проверяем наличие clear_cache:
        if (href.indexOf('?clear_cache=') !== -1 || href.indexOf('&clear_cache=') !== -1) {
        processUrl = true;
    }

        if (processUrl) {
        // Отменяем стандартное поведение перехода
        e.preventDefault();

        // Сначала обрабатываем параметр more
        var newUrl = processMoreParam(href);
        // Затем удаляем clear_cache, если он есть
        newUrl = removeClearCache(newUrl);
