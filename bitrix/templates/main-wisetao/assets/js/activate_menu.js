(function setupPageMenuHoverEffect() {
  document.querySelectorAll('.page-menu').forEach(menuElement => {
    menuElement.addEventListener('mouseover', () => menuElement.classList.add('active'));
    menuElement.addEventListener('mouseout', () => menuElement.classList.remove('active'));
  });
})();

(function initializePageMenuToggleEffect() {
  document.querySelectorAll('.page-menu__menu_category').forEach(button => {
    button.addEventListener('click', () => {
      // Удаляем active у всех кнопок и блоков
      document.querySelectorAll('.page-menu__menu_category').forEach(btn => btn.classList.remove('active'));
      document.querySelectorAll('.page-menu__menu_group_items').forEach(group => group.classList.remove('active'));

      // Добавляем active к текущей кнопке и соответствующему блоку
      const pageId = button.dataset.page;
      button.classList.add('active');
      document.querySelector(`.page-menu__menu_group_items[data-page="${pageId}"]`).classList.add('active');
    });
  });
})();

(function initializeMenuDrops() {
  document.querySelectorAll('.page-menu__menu_link').forEach(link => {
    link.addEventListener('click', (event) => {
      const href = link.getAttribute('href');

      // Если ссылка содержит logistic, позволяем стандартное поведение
      if (href.includes('logistic')) {
        return; // Выходим из обработчика, позволяя событию продолжить выполнение
      }

      // Для всех остальных ссылок предотвращаем переход
      event.preventDefault();

      // Убираем класс active со всех ссылок и блоков
      document.querySelectorAll('.page-menu__menu_link').forEach(l => l.classList.remove('active'));
      document.querySelectorAll('.page-menu__menu_drop').forEach(d => d.classList.remove('active'));

      // Получаем значение data-in-menu текущей ссылки
      const menuValue = link.dataset.inMenu;

      // Добавляем active к текущей ссылке
      link.classList.add('active');

      // Находим соседний блок с соответствующим data-in-content
      const nextDropBlock = link.nextElementSibling;
      if (nextDropBlock &&
          nextDropBlock.classList.contains('page-menu__menu_drop') &&
          nextDropBlock.dataset.inContent === menuValue) {
        nextDropBlock.classList.add('active');
      }
    });
  });
})();

(function initializeDropLinks() {
  // Получаем все ссылки с классом .page-menu__menu_drop_link
  const links = document.querySelectorAll('.page-menu__menu_drop_link');

  links.forEach(link => {
    link.addEventListener('click', (event) => {
      // Получаем значение data-in-level у ссылки
      const levelValue = link.dataset.inLevel;

      // Пытаемся найти блок(и) с data-in-level-content, значение которого совпадает с levelValue
      const matchingContent = document.querySelector(`[data-in-level-content="${levelValue}"]`);

      // Если такой блок найден, значит нужно предотвратить переход по ссылке
      if (matchingContent) {
        event.preventDefault();
        ...
        // Снимаем класс active со всех блоков с data-in-level-content
        const contentBlocks = document.querySelectorAll('[data-in-level-content]');
        contentBlocks.forEach(block => block.classList.remove('active'));

        // Добавляем класс active к текущей ссылке
        link.classList.add('active');

        // Добавляем класс active к найденному соответствующему блоку
        matchingContent.classList.add('active');
      }
      // Если соответствующего блока нет, никаких действий не предпринимается,
      // и ссылка выполняет переход по href, как прописано в разметке.
    });
  });
})();
