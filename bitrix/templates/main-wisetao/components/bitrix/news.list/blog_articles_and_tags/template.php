...
<div class="blog-page__search" data-aos="fade-up" data-ajax_from_blog="<?= $_SESSION['ajax_from_blog'] ?>">
    <form class="blog-page__search_form" method="get"
          action="/<?= $_GET['direct-china']; ?>/<?= $_GET['data-in-menu']; ?>/blog/">
        <input class="blog-page__search_input" type="text" placeholder="按照文章或事件标题搜索" name="search">
        <button class="blog-page__search_btn" type="submit"><img
                    src="<?= SITE_TEMPLATE_PATH ?>/assets/images/search-icon.svg" alt=""></button>
    </form>
</div>
<div class="blog-page__menu" id="<?= $this->GetEditAreaId($arItem['ID']); ?>">
    <ul class="blog-page__menu_list" data-aos="fade-up">
        <? foreach ($arItem["PROPERTIES"]["TAGS"]["VALUE"] as $key => $tag): ?>
            <? if ($key < 6): ?>
                <li><a class="blog-page__menu_list_link"
                       href="/<?= $_GET['direct-china']; ?>/<?= $_GET['data-in-menu']; ?>/blog/?tag=<?= $tag; ?>"><?= $tag; ?></a>
                </li>
            <? endif; ?>
        <? endforeach; ?>
    </ul>
    <div class="blog-page__menu_more" data-aos="fade-up">
        <a class="blog-page__menu_more_link blog-page__menu_more_link_dots" href="#"><img
                    class="blog-page__menu_more_icon" src="<?= SITE_TEMPLATE_PATH ?>/assets/images/3-dots.svg"
                    alt=""></a>
        <? foreach ($arItem["PROPERTIES"]["TAGS"]["VALUE"] as $key => $tag): ?>
            <? if ($key >= 6): ?>
                <a class="blog-page__menu_more_link"
                   href="/<?= $_GET['direct-china']; ?>/<?= $_GET['data-in-menu']; ?>/blog/?tag=<?= $tag; ?>"><?= $tag; ?></a>
            <? endif; ?>
        <? endforeach; ?>
    </div>
...
<div class="blog-page__start">
    <h2 class="group-title" data-aos="fade-up">最新</h2>
    <div class="row">
        <? else : ?>
...
/<?= $_GET["data-in-menu"] ?>
/<?= $_GET["name"] ?>
/<?= $transliteratedName ?>-<?= $arItem["ID"] ?>/"><img itemprop="image" itemscope
                                                        itemtype="https://schema.org/ImageObject"
                                                        class="blog-article-full__img_pic"
...
/<?= $transliteratedName ?>-<?= $arItem["ID"] ?>/" class="blog-article-full__title_text"><?= $arItem["NAME"]; ?></a>
                                <div class="blog-article-full__date" itemprop="datePublished"
                                     datetime="<?= date("d.m.Y", strtotime($arItem["DATE_CREATE"])); ?>"><?= date("d.m.Y", strtotime($arItem["DATE_CREATE"])); ?></div>
                            </div>
                            <p itemprop="description"
                               class="blog-article-full__text"><?= $arItem["PREVIEW_TEXT"]; ?></p>
                        </div>
                    </div>
                </div>
                <?
...
/<?= $_GET["name"] ?>
/<?= $transliteratedName ?>-<?= $arItem["ID"] ?>/">
                        <a class="blog-article__wide_link blog-article__wide_link_new"
                           href="/<?= $_GET["direct-china"] ?>
/<?= $_GET["data-in-menu"] ?>
/<?= $_GET["name"] ?>
/<?= $transliteratedName ?>-<?= $arItem["ID"] ?>/">
                        </a>
                        <div class="blog-article__img" itemprop="image" itemscope
                             itemtype="https://schema.org/ImageObject">
                            <img class="blog-article__img_pic" itemprop="url"
                                 content="<?= $arItem["PREVIEW_PICTURE"]["SRC"]; ?>"
                        ...
                        <div class="blog-article__block">
                            <div class="blog-article__tags">
                                <span class="article-dot article-dot-yellow"></span>
                                <a class="article-tag"
                                   href="#"><? if (!empty($arItem["PROPERTIES"]["TAGS"]["VALUE"])): ?>
...
/<?= $_GET["data-in-menu"] ?>
/<?= $_GET["name"] ?>
/<?= $transliteratedName ?>-<?= $arItem["ID"] ?>/" class="blog-article__title"
                                   itemprop="headline"><?= $arItem["NAME"]; ?></a>
                                <div class="blog-article__date" itemprop="datePublished"
                                     datetime="<?= date("d.m.Y", strtotime($arItem["DATE_CREATE"])); ?>"><?= date("d.m.Y", strtotime($arItem["DATE_CREATE"])); ?></div>
                                <div class="blog-article__text"
                                     itemprop="description"><?= $arItem["PREVIEW_TEXT"]; ?></div>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <? if ($news_count == 0): ?>
            <span data-aos="fade-up" style="margin-left: 16px;">所选类别中暂无新文章。</span>
        <? endif; ?>
    </div>
</div>
<div class="blog-page__articles">
    <h2 class="group-title" data-aos="fade-up">文章</h2>
    <div class="row">
        <?
        $begin_old_articles = true;
        ?>
        <? endif; ?>
        <div class="col-xl-3 col-lg-4 col-md-6 col-12 mb-4" data-aos="fade-up"
...
/<?= $_GET["data-in-menu"] ?>
/<?= $_GET["name"] ?>
/<?= $transliteratedName ?>-<?= $arItem["ID"] ?>/" class="blog-article__title_second"
                           itemprop="headline"><?= $arItem["NAME"]; ?></a>
                    </div>
                </div>
            </div>
...
<div class="blog-page__articles">
    ...
    <div class="blog-page__articles_block" data-aos="fade-up">
        <span>所选类别中暂无新文章。</span>
        <? endif; ?>
    </div>
    <div class="blog-page__articles_more" data-aos="fade-up">
        <div class="blog-page__articles_more_btn">
            <?
            $str_tag_or_search = '';
            ...
            ?>
            <a id="load-more" class="not-clickable" href="/<?= $_GET['direct-china']; ?>/about/blog/?more=<?= $str_tag_or_search ?>"
               style="display: flex;">
                <span style="margin-right: 10px;">还有</span>
